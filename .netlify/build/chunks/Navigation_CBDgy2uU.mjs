...
const $$Navigation = createComponent(($$result, $$props, $$slots) => {
  ...
  const origin = Astro2.request.headers.get("Referer");
  if (!origin) throw new Error("Something went wrong!");
  const isEnglish = !origin.includes("/es/");
  const prefersSpanishAndIsInEn = Astro2.preferredLocale === "es" && isEnglish;
  const pathname = origin.replace(process.env.BASE_URL ?? "http://localhost:4321/", "/").replace("/es/", "/");
  const links = [
    {
      href: getRelativeLocaleUrl(Astro2.currentLocale ?? "en"),
...
No
</button></div>`}<nav> <button id="btn-open" class="nav-open" aria-expanded="false">${renderComponent($$result, "HamburgerMenuIcon", $$HamburgerMenuIcon, {})}</button> <div class="nav-overlay"></div> <div class="nav-content"> <button id="btn-close" class="nav-close">${renderComponent($$result, "CrossIcon", $$CrossIcon, {})}</button> <ul class="nav-links"> ${links.filter(({ href }) => href !== pathname).map(({ href, label, Icon, extraProps }) => renderTemplate`<li style="list-style: none !important;"> <a${addAttribute(href, "href")} style="font-weight:bolder;"> ${label && renderTemplate`<p>${label}</p>`} ${Icon && renderTemplate`${renderComponent($$result, "Icon", Icon, { ...extraProps })}`} </a> </li>`)} </ul> </div> </nav> ${renderScript($$result, "C:/Users/brian/work/briangonzalez/src/components/Navigation.astro?astro&type=script&index=0&lang.ts")}`;
}, "C:/Users/brian/work/briangonzalez/src/components/Navigation.astro", void 0);

const $$file = "C:/Users/brian/work/briangonzalez/src/components/Navigation.astro";
const $$url = undefined;

export { $$Navigation as $, $$file as a, $$url as b, createSvgComponent as c };
